...
// 
// Tags the segment with the preceding segment, unless interrupted by an overlap marked by angle brackets. Does not tag across turn boundaries
  ...
  for each (word in turn.list("transcript")) {
	...
	if (word.label.match(overlapPattern) == null) {
      ...
      for each (segment in word.list("segments")) {
        ...
        if (lastSegment != null) {
          // ...tag the current segment with the previous segment label
          var tag = segment.createTag(thisLayer.id, lastSegment.label);
          log("Tagged segment " + segment.label + " with " + lastSegment.label);
		}
